#!/usr/bin/env node
/* Baut CREW zu EINER HTML-Datei zusammen.
   Aufruf:  node crew/build.js
   Ergebnis:
     crew/dist/index.html          – vollständige Seite (GitHub Pages, USB-Stick, O:-Laufwerk)
     crew/dist/crew-vorschau.html  – gleiche Seite ohne <html>/<head>/<body> (für die Claude-Vorschau)
     crew/dist/sw.js, manifest.webmanifest – Offline-Unterstützung auf GitHub Pages */
'use strict';
const fs = require('fs');
...
const preview = markers(strip(strip(strip(filled, 'HEAD_START', 'HEAD_END'), 'BODY_START', 'BODY_END'), 'TAIL_START', 'TAIL_END'))
  ...
  .replace('<style>', '<style>\nhtml,body{height:100%;margin:0;background:#1b1446;}\n');

fs.mkdirSync(DIST, { recursive: true });
fs.writeFileSync(path.join(DIST, 'index.html'), full);
fs.writeFileSync(path.join(DIST, 'crew-vorschau.html'), preview);

/* Offline: Service Worker + Manifest */
const version = require('crypto').createHash('sha1').update(full).digest('hex').slice(0, 10);
fs.writeFileSync(path.join(DIST, 'sw.js'), `/* CREW Offline-Cache ${version} */
const CACHE = 'crew-${version}';
...
fs.writeFileSync(path.join(DIST, 'manifest.webmanifest'), JSON.stringify({
  name: 'CREW', short_name: 'CREW', start_url: './', display: 'standalone', orientation: 'any',
  background_color: '#1b1446', theme_color: '#1b1446', lang: 'de',
  icons: [{ src: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" rx="22" fill="#ff6b3d"/><text x="50" y="66" font-size="44" text-anchor="middle" font-family="Arial Black,Arial" fill="#1a0b05">CREW</text></svg>'), sizes: 'any', type: 'image/svg+xml' }],
}, null, 2));

const kb = (s) => (Buffer.byteLength(s) / 1024).toFixed(0) + ' KB';
console.log(`CREW gebaut: dist/index.html (${kb(full)}), dist/crew-vorschau.html (${kb(preview)}), ${jsFiles.length} JS-Dateien, ${cssFiles.length} CSS-Dateien.`);
