...
export function Einheiten({
  ...
  onBlaetterZuEldib: (codes: string[]) => void
}) {
  ...
  const ratings: RatingMap = bew.eigene
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (!aktivRef.current || e.key !== '/' || e.ctrlKey || e.metaKey || e.altKey) return
        ...
        <main id="ergebnisse" tabIndex={-1} className="min-w-0 outline-none">
          <div className="mb-3 flex flex-wrap items-end justify-between gap-x-4 gap-y-3">
            <div className="min-w-0">
              <h1 className="disp text-[26px] leading-tight text-ink sm:text-[30px]">Einheiten</h1>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <label className="search search-sm w-[260px] max-w-full">
                <Icon name="search" />
                <span className="sr-only">Einheiten durchsuchen</span>
                <input
                  ref={searchRef}
                  type="search"
                  value={filter.search}
                  ...
                  placeholder="Titel, Thema, ELDiB-Code …"
                  autoComplete="off"
                  spellCheck={false}
                />
              </label>
          ...
          {filter.eldibGoals.length > 0 && (
            <div className="callout callout-info mb-3">
              <Icon name="info" />
              <div>
                Zu {filter.eldibGoals.map(goalText).join(', ')} gibt es auch fertige Arbeitsblätter.{' '}
                <button type="button" className="link-btn" onClick={() => onBlaetterZuEldib(filter.eldibGoals)}>
                  Arbeitsblätter ansehen
                </button>
              </div>
            </div>
          )}
