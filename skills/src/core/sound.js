/* SKILL DECK – Töne (selbst erzeugt mit WebAudio, keine Dateien) und Vorlesen. */
(function () {
  'use strict';
  const SK = window.SK;
  let ctx = null;
  ...
  ['pointerdown', 'touchstart', 'keydown'].forEach((ev) =>
    window.addEventListener(ev, () => { if (SK.settings.sound) ac(); }, { once: true, passive: true })
  );
  ...
  SK.sound = { play, unlock: ac };
  SK.speak = speak;
  SK.stopSpeaking = stopSpeaking;
  SK.canSpeak = () => 'speechSynthesis' in window;
})();
