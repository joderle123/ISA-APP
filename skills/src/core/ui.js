/* SKILL DECK – UI-Bausteine. Alle Missionen bauen ihre Bildschirme hiermit. */
(function () {
  'use strict';
  const SK = window.SK;
  const { h, clear, sleep } = SK.util;
  ...
  function btn(label, onClick, opts) {
    ...
    const b = h('button', { type: 'button', class: cls, id: o.id, disabled: o.disabled, 'aria-label': o.aria },
      o.icon ? SK.icon(o.icon, o.big ? 30 : 24) : null,
      label != null ? h('span', null, label) : null,
      o.iconRight ? SK.icon(o.iconRight, o.big ? 30 : 24) : null);
    b.addEventListener('click', (e) => {
      if (!o.silent) SK.sound.play('tap');
  ...
  function iconBtn(name, onClick, opts) {
    const o = opts || {};
    const b = h('button', { type: 'button', class: 'icon-btn ' + (o.cls || ''), 'aria-label': o.label, title: o.label, id: o.id }, o.text ? o.text : SK.icon(name, 24));
    b.addEventListener('click', (e) => { SK.sound.play('tap'); if (onClick) onClick(e); });
    return b;
  }

  function speakBtn(text, opts) {
    if (!SK.settings.speech || !SK.canSpeak()) return null;
    return iconBtn('speaker', () => SK.speak(typeof text === 'function' ? text() : text), { label: 'Vorlesen', cls: (opts && opts.cls) || '' });
  }
  ...
  function timer(seconds, o) {
    ...
    const tick = () => {
      ...
      if (left <= 5 && left > 0) SK.sound.play('tick');
  ...
  function paddleHint(type, extra) {
    return h('div', { class: 'paddle-hint' }, SK.icon('phone', 22), h('span', null, 'Antwort-Karte: ', h('b', null, PADDLES[type] || type), extra ? ' · ' + extra : ''));
  }

  SK.ui = { screen, btn, iconBtn, speakBtn, say, choice, next, stepper, tally, valuePad, timer, threeTwoOne, modal, confirm, toast, confetti, countUp, paddleHint, PADDLES, stage, overlays };
})();
